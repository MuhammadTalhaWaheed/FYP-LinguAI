...
const LevelScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchUserScores = async () => {
      try {
        const response = await fetch(`http://172.16.73.172:5000/calculate_average_scores`, {
          method: 'POST',
          headers: {
        ...
        if (data?.averages) {
          ...
          const overallScore = (averages.fluency + averages.grammar + averages.vocabulary + averages.cohesion) / 4;
